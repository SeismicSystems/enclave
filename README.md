...
| `centralized-custodian-service` | `seismic-centralized-custodian-service` | Standalone custodian for the network's centralized phase (runs without the other services): epochs >= 1 are Ethereum-wallet-signed security-council deliveries over HTTP (`127.0.0.1:7876`, fronted by the deployment's HTTPS proxy/tunnel) instead of root-key derivations; same Unix-socket API otherwise, root key persisted in a local keyfile. |
