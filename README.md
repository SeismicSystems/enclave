...
| `centralized-custodian-service` | `seismic-centralized-custodian-service` | Standalone custodian for the network's centralized phase (runs without the other services): epochs >= 1 are signed+encrypted security-council deliveries over a TCP port (`:7876`) instead of root-key derivations; same Unix-socket API otherwise, root key persisted in a local keyfile. |
