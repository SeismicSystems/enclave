...
use crate::connection::{CancellableReader, ConnectionControl};
...
pub struct ClientConnection {
    connection: Arc<ConnectionControl>,
...
impl ClientConnection {
    ...
    fn read(&mut self) -> Result<Request, ReadError> {
        let (method, path, version, headers) = {
            ...
            // Reject before constructing a Request (which owns a sequential
            // writer and may eagerly read a small body). Allocating a second
            // writer while that Request is alive would wait on our own writer.
        ...
        let request = crate::request::new_request(
            ...
            *self.remote_addr.as_ref().unwrap(),
            CancellableReader::new(data_source, self.connection.clone()),
            writer,
        )
        .map_err(|e| {
            use crate::request;
        ...
        // return the request
        Ok(request.with_connection(self.connection.clone()))
    }
}
...
impl Iterator for ClientConnection {
    ...
    fn next(&mut self) -> Option<Request> {
        ...
        if self.no_more_requests || self.connection.is_closed() {
            return None;
        }
        ...
        let rq = match self.read() {
            Err(ReadError::UnsupportedVersion) => {
                ...
                let mut response = Response::new_empty(StatusCode(505));
                response.add_connection_close();
                let _ = response
                    .raw_print(&mut writer, HTTPVersion(1, 1), &[], false, None)
                    .and_then(|()| writer.flush());
                // Also cancel when writing fails, before releasing the writer.
                // Do not parse or drain the rejected request's remaining input.
