    ...
    R: Read,
{
    ...
    // Only rejection paths that also shut down the actual socket may emit this
    // header (Request::respond_and_close and parser-level version rejection).
    // User-provided Connection remains ignored.
    pub(crate) fn add_connection_close(&mut self) {
        self.headers
            .push(Header::from_bytes("Connection", "close").unwrap());
    }
