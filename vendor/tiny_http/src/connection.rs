...
/// A raw socket handle shared with requests so cancellation can interrupt I/O
/// without waiting for the HTTP reader/writer (or TLS) locks.
...
/// Also stop reads from userspace buffers after cancellation. In particular,
/// EqualReader must not drain unread body bytes when its request is rejected.
...
impl<R> CancellableReader<R> {
    pub(crate) fn new(inner: R, connection: Arc<ConnectionControl>) -> Self {
