...
pub struct Request {
    connection: Option<Arc<ConnectionControl>>,
...
/// Only the chunk decoder may signal successful message completion (after the
/// terminating chunk). EOF from its source always means truncated framing/data.
/// chunked_transfer otherwise forwards Ok(0) when EOF occurs inside chunk data.
struct ChunkedSource<R>(R);

impl<R: Read> Read for ChunkedSource<R> {
    ...
    W: Write + Send + 'static,
{
    ...
    let reader = if connection_upgrade {
        ...
        Box::new(FusedReader::new(Decoder::new(ChunkedSource(source_data))))
...
impl Request {
    ...
    #[inline]
    pub fn respond<R>(self, response: Response<R>) -> Result<(), IoError>
    where
        R: Read,
    {
        self.respond_with_close(response, false)
    }

    /// Send a final response and shut down the underlying connection before
    /// dropping the request body. Unread bodies are not drained. Use this when
    /// rejecting an oversized, malformed, or incomplete request. Unlike adding
    /// a `Connection` header (which Response ignores), this actually closes the
    /// socket and prevents its buffered input from being parsed as more requests.
    pub fn respond_and_close<R>(self, response: Response<R>) -> Result<(), IoError>
    where
        R: Read,
    {
        self.respond_with_close(response, true)
    }

    fn respond_with_close<R>(mut self, response: Response<R>, close: bool) -> Result<(), IoError>
    where
        R: Read,
    {
        let res = self.respond_impl(response, close);
        if let Some(sender) = self.notify_when_responded.take() {
            sender.send(()).unwrap();
        }
        res
    ...
    fn respond_impl<R>(&mut self, mut response: Response<R>, close: bool) -> Result<(), IoError>
    where
        R: Read,
    {
        ...
        let result = Self::ignore_client_closing_errors(response.raw_print(
            writer.by_ref(),
            self.http_version.clone(),
            &self.headers,
            do_not_send_body,
        ...
        .and_then(|()| Self::ignore_client_closing_errors(writer.flush()));

        // Close even if writing failed, and before releasing the sequential
        // writer or dropping the body reader. The latter may try to drain.
        if close {
    ...
    pub(crate) fn with_connection(mut self, connection: Arc<ConnectionControl>) -> Self {
...
impl Drop for Request {
    fn drop(&mut self) {
        if self.response_writer.is_some() {
            ...
            let _ = self.respond_impl(response, true); // ignoring any potential error
            if let Some(sender) = self.notify_when_responded.take() {
                sender.send(()).unwrap();
            }
        }
...
#[cfg(test)]
mod tests {
    use super::{ChunkedSource, Request};
    use std::io::{ErrorKind, Read};

    #[test]
    fn chunked_source_requires_complete_framing() {
        for truncated in [
            &b"800\r\nhello"[..],        // EOF in chunk data
            &b"5\r\nhello"[..],          // missing chunk CRLF
            &b"5\r\nhello\r\n"[..],      // missing final chunk
            &b"5\r\nhello\r\n0\r\n"[..], // incomplete final chunk
        ] {
            let mut reader = chunked_transfer::Decoder::new(ChunkedSource(truncated));
            assert!(reader.read_to_end(&mut Vec::new()).is_err());
        }
        let mut source = ChunkedSource(&b""[..]);
        assert_eq!(source.read(&mut []).unwrap(), 0);
        assert_eq!(
            source.read(&mut [0]).unwrap_err().kind(),
            ErrorKind::UnexpectedEof
        );

        // The decoder, not underlying socket EOF, marks the message boundary.
        // Preserve bytes belonging to a subsequent keep-alive request.
        let mut source = &b"5\r\nhello\r\n0\r\n\r\nNEXT"[..];
        let mut reader = chunked_transfer::Decoder::new(ChunkedSource(&mut source));
