    ...
    R: Read,
{
    fn drop(&mut self) {
        ...
        // The length is untrusted. Never allocate proportional to an unread
        // request body; cancellation is handled by the underlying reader.
        let mut buf = [0; 8192];

        while remaining_to_read > 0 {
            let count = remaining_to_read.min(buf.len());
...
mod tests {
    ...
    #[test]
    fn dropping_untrusted_length_uses_a_bounded_buffer() {
        struct EofReader;
        ...
        // The original implementation panics on capacity overflow here. This
        // does not need an actual enormous allocation or a network peer.
        let (reader, _) = EqualReader::new(EofReader, usize::MAX);
        drop(reader);
    }

    #[test]
    fn premature_eof_is_not_message_completion() {
        let (mut reader, _) = EqualReader::new(&b"hello"[..], 2048);
        assert_eq!(reader.read(&mut []).unwrap(), 0);
        let mut body = Vec::new();
        assert_eq!(
            reader.read_to_end(&mut body).unwrap_err().kind(),
            std::io::ErrorKind::UnexpectedEof
        );
