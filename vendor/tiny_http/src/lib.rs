...
use connection::{Connection, ConnectionControl};
            ...
            while !inside_close_trigger.load(Relaxed) {
                let new_client = match server.accept() {
                    Ok((sock, _)) => {
                        ...
                        let connection = match sock.try_clone() {
                            Ok(socket) => Arc::new(ConnectionControl::new(socket)),
                            Err(e) => {
                                log::error!("Error cloning client socket: {}", e);
                                continue;
                            }
                        };
                        ...
                        Ok(ClientConnection::new(write_closable, read_closable, connection))
                    }
                    Err(e) => Err(e),
                };
