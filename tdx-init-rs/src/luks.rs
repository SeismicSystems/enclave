...
pub async fn extract_config_from_header(device_path: &std::path::Path) -> Result<InitConfig> {
    let token = extract_luks_token(device_path).await?;

    if let Some(config_data) = token.user_data.get("config") {
        let config: InitConfig = serde_json::from_str(config_data)
            .map_err(TdxInitError::Json)?;
        return Ok(config);
    }
    ...
    Err(TdxInitError::ConfigNotFound("config or metadata not found in LUKS token".to_string()))
