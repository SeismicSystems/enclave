use enclave_contract::{
    deploy_factory, deploy_upgrade_operator_with_multisig, create_multisig_proposal,
    vote_on_multisig_proposal, execute_multisig_proposal, can_execute_multisig_proposal,
    get_multisig_vote_count, check_mrtd_status, print_flush, ANVIL_ALICE_SK, ANVIL_BOB_SK,
};
...
pub async fn test_multisig_upgrade_operator_workflow() -> Result<(), anyhow::Error> {
    ...
    print_flush(format!("MultisigUpgradeOperator deployed at: {:?}\n", multisig_address));
    ...
    print_flush(format!("Proposal created with ID: {:?}, nonce: {}\n", proposal_id, nonce));

    // Wait a bit for the transaction to be processed
    sleep(Duration::from_secs(2));

    // Check initial vote count
    let (approval_count, total_votes) = get_multisig_vote_count(multisig_address, reth_rpc, proposal_id)
        .await
        .map_err(|e| anyhow::anyhow!("failed to get vote count: {:?}", e))?;

    print_flush(format!("Initial vote count - Approvals: {}, Total votes: {}\n", approval_count, total_votes));
    ...
    // Alice votes yes
    vote_on_multisig_proposal(multisig_address, ANVIL_ALICE_SK, reth_rpc, proposal_id, true)
        .await
    ...
    print_flush(format!("Vote count after Alice - Approvals: {}, Total votes: {}\n", approval_count, total_votes));
    assert_eq!(approval_count, 1, "Approval count should be 1 after Alice's vote");
    ...
    print_flush(format!("Can execute proposal after Alice: {}\n", can_execute));
    assert!(!can_execute, "Proposal should not be executable with only 1 vote");
    ...
    print_flush(format!("Vote count after Bob - Approvals: {}, Total votes: {}\n", approval_count, total_votes));
    assert_eq!(approval_count, 2, "Approval count should be 2 after Bob's vote");
    ...
    let can_execute_again = can_execute_multisig_proposal(multisig_address, reth_rpc, proposal_id)
        .await
        .map_err(|e| anyhow::anyhow!("failed to check if proposal can be executed again: {:?}", e))?;

    print_flush(format!("Can execute proposal again: {}\n", can_execute_again));
    assert!(!can_execute_again, "Proposal should not be executable again after execution");
