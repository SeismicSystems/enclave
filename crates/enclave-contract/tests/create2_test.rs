use enclave_contract::{
    deploy_factory, deploy_via_factory_create2, print_flush, ANVIL_ALICE_SK,
};
...
pub async fn test_create2_consistent_addresses() -> Result<(), anyhow::Error> {
    ...
    assert!(second_deploy_res.is_err(), "Second create2 deploy should fail because you deploy to the same address");
