...
pub async fn check_operator(rootfs_hash: Bytes, mrtd: Bytes, rtmr0: Bytes, rtmr3: Bytes) -> Result<bool, anyhow::Error> {
    ...
    let contract_address: Address = OPERATOR_ADDR.parse().expect("const OPERATOR_ADDR does not parse");
...
mod tests {
    ...
    async fn test_check_operator() {
        let thing: Bytes = Bytes::from("00");
        let _result = check_operator(
            thing.clone(),
            thing.clone(),
            thing.clone(),
            thing.clone(),
