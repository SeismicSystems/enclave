...
pub fn stop_reth() -> Result<(), anyhow::Error> {
    supervisorctl_command("stop", SEISMIC_RETH_SERVICE).map_err(
        |e| anyhow::anyhow!("supervisorctl stop reth failed: {}", e),
    )?;
    Ok(())
}
...
pub fn start_reth() -> Result<(), anyhow::Error> {
    supervisorctl_command("start", SEISMIC_RETH_SERVICE).map_err(
        |e| anyhow::anyhow!("supervisorctl start reth failed: {}", e),
    )?;
    Ok(())
}
...
pub fn reth_is_running() -> bool {
    let output = supervisorctl_command("status", SEISMIC_RETH_SERVICE).map_err(
        |e| anyhow::anyhow!("supervisorctl status reth failed: {}", e),
    ).unwrap();
