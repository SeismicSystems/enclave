...
impl KeyManager {
    pub fn new_with_shares(operator_shares: &[OperatorShare]) -> Result<Self> {
        ...
        if operator_shares.len() != 1 && Self::is_tdx_environment() {
            return Err(anyhow!("At least one operator share is required in production"));
        }
        ...
        log::info!("Key manager initialized with TEE share and {} operator shares", operator_shares.len());
    ...
    fn derive_tee_share() -> Result<Vec<u8>> {
        ...
        log::warn!("Not running in TDX environment - using TEST values (not secure for production)");
